...
namespace Mirror
{
	...
	public class SyncVarGameObject : SyncVar<uint>
	{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static uint GetNetId(GameObject go)
		{
			if (go != null)
			{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static GameObject GetGameObject(uint netId)
		{
			var spawned = Utils.GetSpawnedInServerOrClient(netId);
		...
		{
			return new SyncVarGameObject(value);
		}
		...
		{
			return obj is SyncVarGameObject value && this == value;
		}
