...
namespace kcp2k
{
	public class KcpServer
	{
		...
		private IPEndPointNonAlloc reusableClientEP = new IPEndPointNonAlloc(IPAddress.IPv6Any, 0); // where-allocation
#endif
		...
		public void TickIncoming()
		{
			while (socket != null && socket.Poll(0, SelectMode.SelectRead))
			{
				try
				{
					...
					var remoteAddress = reusableClientEP.temp;
					...
					if (msgLength <= rawReceiveBuffer.Length)
					{
						...
						if (!connections.TryGetValue(connectionId, out var connection))
						{
							...
							// we need a new copy!
							var newClientEP = reusableClientEP.DeepCopyIPEndPoint();

							// for allocation free sending, we also need another
							// IPEndPointNonAlloc...
							var reusableSendEP = new IPEndPointNonAlloc(newClientEP.Address, newClientEP.Port);
