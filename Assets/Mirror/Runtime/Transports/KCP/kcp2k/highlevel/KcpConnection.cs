...
namespace kcp2k
{
	...
	public abstract class KcpConnection
	{
		...
		private static int ReliableMaxMessageSize_Unconstrained(uint rcv_wnd)
		{
			return (Kcp.MTU_DEF - Kcp.OVERHEAD - CHANNEL_HEADER_SIZE) * ((int)rcv_wnd - 1) - 1;
		}
		...
		{
			return ReliableMaxMessageSize_Unconstrained(Math.Min(rcv_wnd, Kcp.FRG_MAX));
		}
		...
		private byte[] kcpMessageBuffer;// = new byte[1 + ReliableMaxMessageSize];

		// send buffer for handing user messages to kcp for processing.
		// (avoids allocations).
		// IMPORTANT: needs to be of size:
		//            1 byte header + MaxMessageSize content
		private byte[] kcpSendBuffer;// = new byte[1 + ReliableMaxMessageSize];

		// raw send buffer is exactly MTU.
		private byte[] rawSendBuffer = new byte[Kcp.MTU_DEF];
		...
		private void SendReliable(KcpHeader header, ArraySegment<byte> content)
		{
			...
			{
				Log.Error($"Failed to send reliable message of size {content.Count} because it's larger than ReliableMaxMessageSize={ReliableMaxMessageSize(kcp.rcv_wnd)}");
			}
		}
