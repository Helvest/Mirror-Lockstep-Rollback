...
namespace Mirror.RemoteCalls
{
	...
	public static class RemoteCallHelper
	{
		...
		{
			if (GetInvokerForHash(cmdHash, invokeType, out var invoker) && invoker.invokeClass.IsInstanceOfType(invokingType))
			{
		...
		{
			if (GetInvokerForHash(cmdHash, MirrorInvokeType.Command, out var invoker) && invoker.invokeClass.IsInstanceOfType(invokingType))
			{
		...
		{
			if (cmdHandlerDelegates.TryGetValue(cmdHash, out var invoker))
			{
