...
buffer.Values[amount - 1].localTimestamp <= threshold;
			...
			where T : Snapshot
		{
			...
			return HasAmountOlderThan(buffer, time - bufferTime, 2);
		}
			...
			where T : Snapshot
		{
			...
			return HasAmountOlderThan(buffer, time - bufferTime, 3);
		}
				...
				where T : Snapshot
		{
			...
			GetFirstSecondAndDelta(buffer, out var first, out var second, out double delta);
