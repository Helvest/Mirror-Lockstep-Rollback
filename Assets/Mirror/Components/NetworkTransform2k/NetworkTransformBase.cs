...
namespace Mirror
{
	public abstract class NetworkTransformBase : NetworkBehaviour
	{
		...
		protected virtual void OnClientToServerSync(Vector3? position, Quaternion? rotation, Vector3? scale)
		{
			...
			{
				position = serverBuffer.Count > 0 ? serverBuffer.Values[serverBuffer.Count - 1].position : targetComponent.localPosition;
			}

			if (!rotation.HasValue)
			{
				rotation = serverBuffer.Count > 0 ? serverBuffer.Values[serverBuffer.Count - 1].rotation : targetComponent.localRotation;
			}

			if (!scale.HasValue)
			{
				scale = serverBuffer.Count > 0 ? serverBuffer.Values[serverBuffer.Count - 1].scale : targetComponent.localScale;
			}

			// construct snapshot with batch timestamp to save bandwidth
			var snapshot = new NTSnapshot(
				timestamp,
				NetworkTime.localTime,
		...
		protected virtual void OnServerToClientSync(Vector3? position, Quaternion? rotation, Vector3? scale)
		{
			...
			if (!position.HasValue)
			{
				position = clientBuffer.Count > 0 ? clientBuffer.Values[clientBuffer.Count - 1].position : targetComponent.localPosition;
			}

			if (!rotation.HasValue)
			{
				rotation = clientBuffer.Count > 0 ? clientBuffer.Values[clientBuffer.Count - 1].rotation : targetComponent.localRotation;
			}

			if (!scale.HasValue)
			{
				scale = clientBuffer.Count > 0 ? clientBuffer.Values[clientBuffer.Count - 1].scale : targetComponent.localScale;
			}
		...
		private void UpdateServer()
		{
				...
				(!clientAuthority || IsClientWithAuthority))
			{
				...
				var snapshot = ConstructSnapshot();
#if onlySyncOnChange_BANDWIDTH_SAVING
			...
			if (clientAuthority && !hasAuthority)
			{
				...
				if (SnapshotInterpolation.Compute(
					...
					out var computed))
				{
					var start = serverBuffer.Values[0];
					var goal = serverBuffer.Values[1];
		...
		private void UpdateClient()
		{
			...
			else
			{
				...
				if (SnapshotInterpolation.Compute(
					...
					out var computed))
				{
					var start = clientBuffer.Values[0];
					var goal = clientBuffer.Values[1];
					ApplySnapshot(start, goal, computed);
				}
			}
		}
		...
		public override void OnDeserialize(NetworkReader reader, bool initialState)
		{
			...
			if (initialState)
			{
				if (syncPosition)
				{
					targetComponent.localPosition = reader.ReadVector3();
				}

				if (syncRotation)
				{
					targetComponent.localRotation = reader.ReadQuaternion();
				}

				if (syncScale)
				{
					targetComponent.localScale = reader.ReadVector3();
				}
			}
		}
		...
		protected virtual void OnGUI()
		{
			...
			var point = Camera.main.WorldToScreenPoint(targetComponent.position);
