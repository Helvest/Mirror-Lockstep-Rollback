...
namespace FlyingWormConsole3.LiteNetLib.Utils
{
	...
	public sealed class FNVHasher : NetSerializerHasher
	{
		...
		{
			if (_hashCache.TryGetValue(type, out ulong hash))
			{
				return hash;
			}
	...
	public sealed class NetSerializer
	{
		...
		private StructInfo RegisterInternal<T>() where T : class
		{
			...
			if (_cache.TryGetValue(nameHash, out var info))
			{
				return info;
			}
			...
			for (int i = 0; i < props.Length; i++)
			{
				...
				else
				{
					...
					if (_registeredCustomTypes.TryGetValue(propertyType, out var registeredCustomType))
					{
		...
		private StructInfo ReadInfo(NetDataReader reader)
		{
			...
			if (!_cache.TryGetValue(hash, out var info))
			{
